...
model->setHeaderData(10, Qt::Horizontal, QObject::tr("Prog_iD"));
    return model;
}

QSqlQueryModel* Employees:: recherche( int  id)
 {
     QSqlQueryModel * model= new QSqlQueryModel();
     QSqlQuery query;
     QString res= QString::number(id);
     query.bindValue(":id", res);
     model->setQuery("select * from Employees where Employee_ID = :id");
...
query.prepare("Delete from etudiant where Employee_ID = :id ");
query.bindValue(":id", res);
return    query.exec();
}
